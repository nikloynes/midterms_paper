...
# NL, 23/01/19 -- slightly amended to tune models for vote==rep
# NL, 11/02/19 -- fixed an error that was wrongly assigning docvars to corpora, aka 02X_tune_models_reps.R
...
for(i in 1:length(models)){
  ...
  for(l in 1:length(mention_text_voters)){
    voters_docvars[l] <- voting_panel_users_df$rep_vote[voting_panel_users_df$user_id==names(mention_text_voters)[l]]
  ...
  # voters_docvars <- voting_panel_users_df$rep_vote
...
write.csv(model_specifications_df, paste0(DATA_PATH, "making_forecast/data/model_specifications/all_waves_reps_model_specs.csv"))
beep(sound = 8)
