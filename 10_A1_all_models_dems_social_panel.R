...
# NL, 11/02/19  -- fixed error where docvars were wrongly assigned to corpora, aka 03_all_models_no_waves_dems.R
...
for(i in 1:nrow(model_parameters_df)){
  ...
  for(l in 1:length(voters_mention_text)){
    voters_docvars[l] <- voting_panel_users_df$dem_vote[voting_panel_users_df$user_id==names(voters_mention_text)[l]]
...
panel_users_out_df <- panel_users_df %>% select(
  user_id, admin, ideology_groups, dem_vote, prob_dem_trump_model, 
  prob_dem_kavanaugh_model, prob_dem_midterms_model, 
  prob_dem_democrats_model, prob_dem_republicans_model
)

write.csv(panel_users_out_df, paste0(DATA_PATH, "making_forecast/data/all_models_no_waves_dems.csv"))
beep(sound = 8)