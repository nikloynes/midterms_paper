# 06b_all_models_megaloop_reps.R
...
# NL, 11/02/19  no waves, running for ctrl panel. REPS
...
# ctrl_panel_df$rep_vote[ctrl_panel_df$vote_any_wave=="D"] <- 1
# ctrl_panel_df$rep_vote[ctrl_panel_df$vote_any_wave=="R"] <- 0
...
for(i in 1:nrow(model_parameters_df)){
  ...
  for(l in 1:length(voters_mention_text)){
    voters_docvars[l] <- voting_ctrl_panel_df$rep_vote[voting_ctrl_panel_df$user_id==names(voters_mention_text)[l]]
...
ctrl_panel_out_df <- ctrl_panel_df %>% select(
  user_id, state, ideology_groups, rep_vote, prob_rep_trump_model, 
  prob_rep_kavanaugh_model, prob_rep_midterms_model, 
  prob_rep_democrats_model, prob_rep_republicans_model
)

write.csv(ctrl_panel_out_df, paste0(DATA_PATH, "all_models_no_waves_reps.csv"))
beep(sound = 8)