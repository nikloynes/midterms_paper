# 06c_all_models_megaloop_dems-CPonSP.R
...
model_parameters_df <- read.csv(paste0(CTRL_PANEL_DATA_PATH, "model_specifications/all_waves_dems_model_specs.csv"),
                                stringsAsFactors = F) %>% mutate(
                                  X = NULL
                                )
...
for(i in 1:nrow(model_parameters_df)){
  ...
  mentions_df <- read.csv(paste0(CTRL_PANEL_DATA_PATH, "wave_everything_data/", 
    ...
    filter(user_id %in% unique(voting_ctrl_panel_df$user_id)) %>% 
  ...
  corpus_labelled_ctrlpanel <- corpus(x = mention_text,
                                      docvars = data.frame(voters_docvars))
  
  # STEP 4 - pull unlabelled text for social panel
  ...
  corpus_socpan <- corpus(mention_text)
  
  # STEP 6 - merge corpora and build DFM
  all_users_corpus <- corpus_labelled_ctrlpanel + corpus_socpan
  ...
  # STEP 7 - train model
  model <- textmodel_nb(x = all_users_dfm[1:ndoc(corpus_labelled_ctrlpanel)], 
                        y = docvars(all_users_dfm)[1:ndoc(corpus_labelled_ctrlpanel), 1], 
                        prior = "uniform")
  ...
  predicted_values <- as.numeric(
    as.character(predict(
      model, 
      newdata = all_users_dfm[(ndoc(corpus_labelled_ctrlpanel)+1):ndoc(all_users_dfm)])))
  ...
  socpan_users_df <- left_join(socpan_users_df, predicted_probabilities_df, by = "user_id") %>% mutate(
    prob = round(prob, digits = 4)
  )
  
  varname <- paste0("prob_dem_CPonSP_", model_parameters_df$model[i], "_model")
  socpan_users_df[varname] <- socpan_users_df$prob
  socpan_users_df$prob <- NULL
  
}

socpan_out_df <- socpan_users_df %>% select(
  user_id, admin, ideology_groups, prob_dem_CPonSP_trump_model, 
  prob_dem_CPonSP_kavanaugh_model, prob_dem_CPonSP_midterms_model, 
  prob_dem_CPonSP_democrats_model, prob_dem_CPonSP_republicans_model
)

write.csv(socpan_out_df, paste0(SOCPAN_DATA_PATH, "making_forecast/data/CPonSP_no_waves_dems.csv"))