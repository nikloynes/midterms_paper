...
# NL, 10/02/19 -- update to adapt this to ctrl_panel, for dems
...
# ctrl_panel_df$dem_vote[is.na(ctrl_panel_df$dem_vote)] <- 0
...
for(i in 1:length(models)){
  ...
  for(l in 1:length(mention_text_voters)){
    voters_docvars[l] <- voting_ctrl_panel_df$dem_vote[voting_ctrl_panel_df$user_id==names(mention_text_voters)[l]]
...
write.csv(model_specifications_df, paste0(DATA_PATH, "model_specifications/all_waves_dems_model_specs.csv"))
beep(sound = 8)
