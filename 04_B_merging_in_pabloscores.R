...
# DATA IN
old_social_panel_df <- read.csv("panel_demographics_ideology.csv")
...
pabloscores_df <- read.csv("nik_user_set_ideal_points (1).csv", header = FALSE)
...
social_panel_df$ideology <- NULL
social_panel_ideology_df <- left_join(social_panel_df, pabloscores_df, by = "user_id")
...
write.csv(social_panel_ideology_df, "social_panel_ideology.csv")
