...
# NL, 11/02/19 -- fixed an error that was assigning wrong docvars to corpora, aka 02X_tune_models_dems.R

# TO DO -- AUDIT MEMORY HOG WITH gc() AT STRATEGIC POINTS IN THE LOOP
...
panel_users_df <- read.csv(paste0(DATA_PATH, "making_forecast/data/panel_users_all_declared_voters.csv"),
...
indices <- sample(x = 1:nrow(voting_panel_users_df), size = nrow(voting_panel_users_df)*0.2, replace = F)

holdout_set_df <- voting_panel_users_df[indices,]
voting_panel_users_df <- voting_panel_users_df[-indices,]
...
for(i in 1:length(models)){
  ...
  mentions_df <- read.csv(paste0(DATA_PATH, "wave_everything_data/", models[i], "_mention_tweets.csv"), colClasses = "character") %>% select(
    ...
    user_id %in% c(voting_panel_users_df$user_id, holdout_set_df$user_id)
  )
  ...
  for(j in 1:length(unique(voting_panel_users_df$user_id))){
    
    user_tweets_df <- mentions_df %>% filter(
      user_id == unique(voting_panel_users_df$user_id)[j]
    )
    ...
    names(mention_text_voters)[j] <- unique(voting_panel_users_df$user_id)[j]
  ...
  for(l in 1:length(mention_text_voters)){
    voters_docvars[l] <- voting_panel_users_df$dem_vote[voting_panel_users_df$user_id==names(mention_text_voters)[l]]
  ...
  # 
  # corpus_voters <- corpus(x = mention_text_voters[which(names(mention_text_voters) %in% voting_panel_users_df$user_id)],
  #                               docvars = data.frame(voters_docvars))
  ...
  for(l in 1:length(mention_text_holdout)){
    holdout_docvars[l] <- holdout_set_df$dem_vote[holdout_set_df$user_id==names(mention_text_holdout)[l]]
  ...
  # holdout_docvars <- holdout_set_df$dem_vote
  ...
  all_corpus[["dem_vote_dummy"]] <- c(unname(voters_docvars), unname(holdout_docvars))
...
write.csv(model_specifications_df, paste0(DATA_PATH, "making_forecast/data/model_specifications/all_waves_dems_model_specs.csv"))
beep(sound = 8)
